...
func TestConfigClusterMembershipReload(t *testing.T) {
	...
	// Remove the cluster name of first server with a reload and back to using a dynamic one.
	//
	// In order to do that we need to first remove the static routes from
	// the other members of the AB cluster otherwise, the A node will go back
	// to using the AB cluster name because of the static route CONNECT.
	...
	endpoint3 := fmt.Sprintf("nats://%s:%d", "127.0.0.1", s3.Addr().(*net.TCPAddr).Port)
